...
with pm.Model() as model:
    ...
    LB = pm.expand_packed_triangular(2, packed_LB) # convert a packed triangular matrix into a two dimensional array
        ...
        for fi, curr_fix in enumerate(all_fix):
            ...
            try:
                ...
                with model:
                    ...
                    approx = pm.fit(n=20000, method=pm.ADVI()) # approximate the posterior for that fixation
    ...
    if parallel:
        ...
        with Pool(n_processes) as p:
            multiple_results = [
                p.apply_async(
                    ...
                    args=(sub_data[:n_train], sub, parameters, lib, method, "train"),
                )
        ...
        with Pool(n_processes) as p:
            multiple_results = [
                p.apply_async(
                    extract_features_sub,
                    args=(sub_data[n_train:], sub, parameters, lib, method, "test"),
                )
                for sub, sub_data in enumerate(data)
            ]
            _ = [res.get() for res in multiple_results]
        ...
        for sub, sub_data in enumerate(data):
            extract_features_sub(
                sub_data[:n_train], sub, parameters, lib, method, dset="train"
            )
            extract_features_sub(
                sub_data[n_train:], sub, parameters, lib, method, dset="test"
            )


def load_cerf():
    scanpath = []
    data = sio.loadmat("../Gaze_4_behavioural_biometrics/datasets/FIFA/fixations.mat")['sbj'][0]
    for subject in range(8): 
        ...
        scanpath.append(sub_scanpath) # each trial is a bi-dimensional array with x/y coordinates of points [x,y]
    parameters = {
        'distance':0.8,
        'width':0.43,
        'height':0.31,
